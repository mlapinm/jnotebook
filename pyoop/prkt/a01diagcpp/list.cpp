...
//	fChoice(numShow);		//????
	...
	{
		copyUnitsAll();
		dlgCust->tabPiu->updateDialog();
	}
	if(numShow==7)
	{
		copyUnitsAll();
		dlgCust->tabPdo->iChoiceRls = iChoiceRls;
		dlgCust->tabPdo->updateDialog();
	}
	QString str="111";
...
//	a1 = unit->i18byteReceived;
        ...
        str = codecChar->toUnicode(chName,k);
        strWhole =str;
		strWhole.append('/');
/////////
		for(int i=6;i<12;i++)
