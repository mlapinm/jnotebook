...
void Dialog::slTabCurIndChange(int i)
{
	i=ui.tabWidget_2->currentIndex();
	if(i==0)
	rastr->bShow=true;
	else
	rastr->bShow=false;
...
int aaa=0;
//	currentChanged;
//	ui.tabWidget_2->currentChanged(int);
//
...
//common
	connect(ui.tabWidget_2,SIGNAL(currentChanged(int)),this,SLOT(slTabCurIndChange(int)));
	connect(ui.tabWidget,SIGNAL(currentChanged(int)),this,SLOT(slTabCurIndChange(int)));
	...
	connect(ui.grBoRzrBlk,SIGNAL(clicked()),this,SLOT(slGrBoRzrBlk()));
	connect(ui.chBoConsist27,SIGNAL(clicked()),this,SLOT(slChBoConsist27()));
...
//имена
	...
	connect(lineEditRls0DistanceAxisVPP2,SIGNAL(editingFinished()),this,SLOT(fLineEditRls0DistanceAxisVPP2()));
	connect(lineEditRls0PointPlanting2,SIGNAL(editingFinished()),this,SLOT(fLineEditRls0PointPlanting2()));

	connect(ui.liEdRls0TurnCorr,SIGNAL(editingFinished()),this,SLOT(slLiEdRls0TurnCorr()));
	connect(ui.liEdRls0TurnCorr,SIGNAL(textChanged ( const QString&)),this,SLOT(fLineEditRls0Arguments(const QString&)));
	connect(ui.liEdRls1TurnCorr,SIGNAL(editingFinished()),this,SLOT(slLiEdRls1TurnCorr()));
	connect(ui.liEdRls1TurnCorr,SIGNAL(textChanged ( const QString&)),this,SLOT(fLineEditRls1Arguments(const QString&)));
...
//rls1
	...
	connect(lineEditRls1DistanceAxisVPP2,SIGNAL(editingFinished()),this,SLOT(fLineEditRls1DistanceAxisVPP2()));
...
//пороги
	connect(lineEditCompensZ,SIGNAL(textChanged(QString)),this,SLOT(fLineEditCompensZ()));
	connect(lineEditCompensPass,SIGNAL(textChanged(QString)),this,SLOT(fLineEditCompensPass()));
	connect(lineEditCompensAct,SIGNAL(textChanged(QString)),this,SLOT(fLineEditCompensAct()));
	connect(lineEditCompensInput1,SIGNAL(textChanged(QString)),this,SLOT(fLineEditCompensInput1()));
	connect(lineEditCompensInput2,SIGNAL(textChanged(QString)),this,SLOT(fLineEditCompensInput2()));
	...
	connect(lineEditRiftAct2,SIGNAL(editingFinished()),this,SLOT(fLineEditRiftAct2()));
	...
	connect(ui.widget_3,SIGNAL(callMoveEdge2()),this,SLOT(fMoveEdgeRiftAct2()));
...
{
//64l максимальная длительность одного блока с несколькими лучами
	int iAddr =64;
	PLineEdit* lineEdit=lineEditMaxLengthUnit;
	QString str1,str2;
	ushort b1=0,b2=0;
	str1=lineEdit->text();
	bool ok=true;
...
void Dialog::slGrBoRzrBlk()
{
//grBoRzrBlk	//бит 15: разрешение блокировки(0-блокировка запрещена, 1-блокировка
	int iAddr =34;
	int b1=0;
	b1 = unitReceive->basicSetting[iAddr];
	fWordWr(b1);
	if(ui.grBoRzrBlk->isChecked())
	{
		wordWr.bit15=1;
	}else
	{
...
void Dialog::slChBoConsist27()
{
//chBoConsist27 //бит 0: разрешающее состояние сигнала "выбор РЛС"
	int iAddr =34;
	int b1=0;
	b1 = unitReceive->basicSetting[iAddr];
	fWordWr(b1);
	if(ui.chBoConsist27->isChecked())
	{
		wordWr.bit0=1;
	}else
	{
...
{
//64h время ожидания открытого блока
	int iAddr =64;
	PLineEdit* lineEdit=lineEditMaxWaitRay;
	QString str1,str2;
...
{
//65l значение селектора длительности МА10° для РСП-10 в 1/48 мкс (8 разрядов)
	int iAddr =65;
	PLineEdit* lineEdit=lineEditSelectorMa10;
	QString str1,str2;
	ushort b1=0,b2=0;
	str1=lineEdit->text();
	bool ok=true;
...
//72    198     326 НП 2 процент МУ глиссады 7°
		...
		int tmp = to2byteP100(blocMeas->muG7);
		unitReceive->setSelectedParameters(iChoiceRls,iAddr,blocMeas->muG7);
		unitSend->setSelectedParameters(iChoiceRls,iAddr,blocMeas->muG7);
	}
...
void Dialog::fLineEditRls0DistanceAxisVPP2()
{
	int iAddr = 39;
	QLineEdit* lineEdit = lineEditRls0DistanceAxisVPP2;
	int b1;
	QString str1,str2;
	str1=lineEdit->text();
	b1=str1.toInt();
...
{
	int iAddr = 47;
	PLineEdit* lineEdit = lineEditRls0TurnKO;
	int b1;
	QString str,str2,str3,strend;
	str = lineEdit->text();
        uint ival=0;double deg=0;
	if(str.contains(QChar('.'))||str.contains(QChar(','))||str.contains(QChar('°')))
	{
		while(str.contains(QChar('°')))
		{
			str.chop(1);
...
void Dialog::fLineEditRls1DistanceAxisVPP2()
{
	int iAddr = 39;
	QLineEdit* lineEdit = lineEditRls1DistanceAxisVPP2;
	int b1;
	QString str1,str2;
	str1=lineEdit->text();
	b1=str1.toInt();
...
void Dialog::slLiEdRls0TurnCorr()
{
	int iAddr = 11;
	PLineEdit* lineEdit = ui.liEdRls0TurnCorr;
	int b1;
	QString str,str2,str3,strend;
	str = lineEdit->text();
        uint ival=0;double deg=0;
...
//2
void Dialog::slLiEdRls1TurnCorr()
{
	int iAddr = 11;
	PLineEdit* lineEdit = ui.liEdRls1TurnCorr;
	int b1;
	QString str,str2,str3,strend;
	str = lineEdit->text();
        uint ival=0;double deg=0;
...
{
	int iAddr = 47;
	PLineEdit* lineEdit = lineEditRls1TurnKO;
	int b1;
	QString str,str2,str3,strend;
	str = lineEdit->text();
        uint ival=0;double deg=0;
...
void Dialog::fLineEditRiftAct2()
{
	int iAddr = 19;
	int b1,b2;
	QString str=lineEditRiftAct2->text();
	b1=str.toInt();
	if(b1<1024 && b1>=0)
	{
		lineEditRiftAct2->setText(QString("%1").arg(b1));
		unitReceive->setSelectedParameters(iChoiceRls,iAddr,b1);
		unitSend->setSelectedParameters(iChoiceRls,iAddr,b1);
		if(isSetPressEnter())
			lineEditRiftAct2->selectAll();
		ui.widget_3->setLevelEdgeValue(1,b1);
//		netExchange->waiteCopyTwin(iAddr,1,2);		////////////////////
	}
	else
	{
		if(b1<0)b2=0;
		if(b1>1023)b2=1023;
		lineEditRiftAct2->setText(QString("%1").arg(b2));
		lineEditRiftAct2->setText(QString("%1").arg(b2));
		ui.widget_3->setLevelEdgeValue(1,b2);
	}
}
void Dialog::fMoveEdgeRiftZ()
{
  ...
  unitReceive->setSelectedParameters(iChoiceRls,14,a1);
  unitSend->setSelectedParameters(iChoiceRls,14,a1);
	lineEditRiftZ->setFocus();
}
void Dialog::fMoveEdgeRiftPass1()
{
  ...
  unitReceive->setSelectedParameters(iChoiceRls,15,a1);
  unitSend->setSelectedParameters(iChoiceRls,15,a1);
	lineEditRiftPass1->setFocus();
}
void Dialog::fMoveEdgeRiftAct1()
{
  ...
  unitReceive->setSelectedParameters(iChoiceRls,18,a1);
  unitSend->setSelectedParameters(iChoiceRls,18,a1);
	lineEditRiftAct1->setFocus();
}
void Dialog::fMoveEdgeRiftAct2()
{
	int a1;QString str;
	a1=ui.widget_3->levelEdgeValue(1);
	str = QString("%1").arg(a1);
	lineEditRiftAct2->setText(str);
  unitReceive->setSelectedParameters(iChoiceRls,19,a1);
  unitSend->setSelectedParameters(iChoiceRls,19,a1);
	lineEditRiftAct2->setFocus();
}
...
void Dialog::wheelEvent(QWheelEvent *e)
{
	...
	if(focus == ui.lineEdit_42)
	{
		int num = ui.lineEdit_42->text().toInt();
		ui.lineEdit_42->setText(e->delta() > 0 ? QString::number(num+1) : QString::number(num-1));
		fLineEditRiftAct2();
	}
	if(focus == lineEditRiftMai)
	{
...
//31.08.2017
//dialog.cpp dialog2.cpp dialog.h
//64l максимальная длительность одного блока с несколькими лучами
//64l максимальная длительность одного блока с несколькими лучами
//64h время ожидания открытого блока
//65l значение селектора длительности МА10° для РСП-10 в 1/48 мкс (8 разрядов)
