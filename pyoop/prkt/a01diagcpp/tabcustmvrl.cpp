...
//218	дальность точки стояния, метры
	iAddr0 =218 -128;
...
//219 азимут точки стояния, минуты (градусы ? 60)
	iAddr0=219 -128;
...
//234	дальность точки стояния, метры
	iAddr0 =234 -128;
	a1 = unitReceive->selectedParameters0[iAddr0];
	str = QString("%1").arg(a1);
	if(!ui.liEdMVRL2PntDistance->hasFocus())
		ui.liEdMVRL2PntDistance->setText(str);
//235 азимут точки стояния, минуты (градусы ? 60)
	iAddr0=235 -128;
	a0 = unitReceive->selectedParameters0[iAddr0];
	a1 = a0/60;
	a2 = a0 - (a1*60);
...
//250	дальность точки стояния, метры
	iAddr0 =250 -128;
	a1 = unitReceive->selectedParameters0[iAddr0];
	str = QString("%1").arg(a1);
	if(!ui.liEdSt2ThPntDistance->hasFocus())
		ui.liEdSt2ThPntDistance->setText(str);
//251 азимут точки стояния, минуты (градусы ? 60)
	iAddr0=251 -128;
	a0 = unitReceive->selectedParameters0[iAddr0];
	a1 = a0/60;
	a2 = a0 - (a1*60);
