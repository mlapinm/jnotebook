...
//	unitReceive->function();
	...
	dlgCust->tabPiu->unitReceive=unitReceive;
	dlgCust->tabPiu->unitSend=unitSend;
	dlgCust->tabPdo->unitReceive=unitReceive;
	dlgCust->tabPdo->unitSend=unitSend;
...
//10	138	266	переключаемые параметры РЛС
	...
	lineEditRls0DistanceAxisVPP2= ui.lineEdit_254;
	lineEditRls1DistanceAxisVPP2= ui.lineEdit_255;
...
void Dialog::showNum(int v)//0-надпись,1-диагн,2-PSP1,3-PS5,4-PSM,5-PSP,6-PIU,7-PVI
{
	if(v>7)v=0;
	switch(v)
	{
...
//        int tmpCurrent;
	...
	else
	{
		...
		if(boardCurrent->typeT==4)
			showNum(5);
		if(boardCurrent->typeT==6)
			showNum(6);
		if(boardCurrent->typeT==7)
			showNum(7);
...
//64l	максимальная длительность одного блока с несколькими лучами, мс
	iAddr0 =64;
	a1 = unitReceive->basicSetting[iAddr0];
	a1 = a1&0xFF;
	str = QString("%1").arg(a1);
...
//точка стояния
	...
	a1=unitReceive->isSelectedParameters(iChoiceRls,10);			//9.11.2016
	fWord(a1);
...
//11	139	267	доворот до коррекции
	a1 = unitReceive->isSelectedParameters(0,11);
	a3=to12bit(a1);
	a3&=0xfff;
	d1=a3*360./4096.;
	if(d1>180.&&d1<360.)
	...
	if(!ui.liEdRls0TurnCorr->hasFocus())
		ui.liEdRls0TurnCorr->setText(str);
	a1 = unitReceive->isSelectedParameters(1,11);
	a3=to12bit(a1);
	a3&=0xfff;
	d1=a3*360./4096.;
	if(d1>180.&&d1<360.)
	...
	str = QString("%1°/").arg(d1,0,'f',1)+QString("%1").arg(a3);
	str.replace(str.indexOf(QChar('.')),1,QChar('.'));
	if(!ui.liEdRls1TurnCorr->hasFocus())
		ui.liEdRls1TurnCorr->setText(str);
//
...
//34	162	290	удаление антенны от продольной оси ВПП (число знаковое), м (напр.1)
	str = QString("%1").arg((short)unitReceive->isSelectedParameters(0,34));
	str2 = QString("%1").arg((short)unitReceive->isSelectedParameters(1,34));
...
//39	167	295	удаление антенны от продольной оси ВПП (число знаковое), м (напр.2)
	str = QString("%1").arg((short)unitReceive->isSelectedParameters(0,39));
	str2 = QString("%1").arg((short)unitReceive->isSelectedParameters(1,39));
	if(!lineEditRls0DistanceAxisVPP2->hasFocus())
		lineEditRls0DistanceAxisVPP2->setText(str);
	if(!lineEditRls1DistanceAxisVPP2->hasFocus())
		lineEditRls1DistanceAxisVPP2->setText(str2);
//42	165	293	удаление точки посадки от антенны, м (напр.2)
...
//47	175	303	доворот КО
	...
	a1 = unitReceive->isSelectedParameters(1,47);
	a3=to12bit(a1);
	a3&=0xfff;
	d1=a3*360./4096.;
	if(d1>180.&&d1<360.)
	...
	if(!lineEditRls1TurnKO->hasFocus())
		lineEditRls1TurnKO->setText(str);
...
//19	147	275	порог видео акт 2		THR5		CHK20	ЦАП1 канал 5	8
	a1 = unitReceive->isSelectedParameters(iChoiceRls,19);
	str = QString("%1").arg(a1);
	if(!lineEditRiftAct2->hasFocus())
	{
		if(a1<1024&&a1>=0)
		{
			lineEditRiftAct2->setText(str);
			ui.widget_3->setLevelEdgeValue(1,a1);
		}
		else
		{
			a1=0;
